...
@Component
public class DisableAddNewTaskToBarUpdateHandler extends AbstractSettingsUpdateHandler {
    protected DisableAddNewTaskToBarUpdateHandler(UserService userService, TaskService taskService, TaskLogService taskLogService) {
        super(userService, taskService, taskLogService);
    }

    @Override
    protected void handle(Update update, SendMessage answer, User user) {
        user.setAddNewTasksToButtonBar(false);
