...
@Component
public class EnablePrecontactModeUpdateHandler extends AbstractSettingsUpdateHandler {
    protected EnablePrecontactModeUpdateHandler(UserService userService, TaskService taskService, TaskLogService taskLogService) {
        super(userService, taskService, taskLogService);
    }

    @Override
    protected void handle(Update update, SendMessage answer, User user) {
        user.setPrecontactMode(true);
