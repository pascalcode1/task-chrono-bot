...
@Component
public class DisableWeekProgressUpdateHandler extends AbstractSettingsUpdateHandler {
    protected DisableWeekProgressUpdateHandler(UserService userService, TaskService taskService, TaskLogService taskLogService) {
        super(userService, taskService, taskLogService);
    }

    @Override
    protected void handle(Update update, SendMessage answer, User user) {
        user.setWeekHoursStat(false);
