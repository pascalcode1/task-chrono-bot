...
@Component
public class EnableAddNewTaskToBarUpdateHandler extends AbstractSettingsUpdateHandler {
    protected EnableAddNewTaskToBarUpdateHandler(UserService userService, TaskService taskService, TaskLogService taskLogService) {
        super(userService, taskService, taskLogService);
    }

    @Override
    protected void handle(Update update, SendMessage answer, User user) {
        user.setAddNewTasksToButtonBar(true);
